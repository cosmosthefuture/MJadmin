...
export default function AdminProfile() {
  ...
  const onSubmit = async (data: formTypes) => {
    try {
      const response = await updateAdminProfile({ name: data.name, email: data.email }).unwrap();
      if (response.message) {
        ...
        <div>
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-2 lg:gap-7 2xl:gap-x-32">
            ...
            <div>
              ...
              <p className="text-sm font-medium text-gray-800 dark:text-white/90">{role}</p>
            </div>
          </div>
        </div>
      </div>
