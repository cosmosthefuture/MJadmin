...
import { useGetManualDepositRequestsQuery } from "@/redux/features/deposit/DepositApiSlice";
...
export default function ManualDepositTable() {
  const dispatch = useAppDispatch();
  const currentPage = useAppSelector((state) => state.pagination.currentPage);
  const [searchText, setSearchText] = useState("");
  const debouncedSearchText = useDebounce(searchText);

  const hadAdminViewPermission = useCheckPermission("admin_view");

  const { data, isLoading } = useGetManualDepositRequestsQuery({
    page: currentPage,
      ...
      <div className="max-w-full overflow-x-auto">
        <div className="min-w-[800px]">
          ...
          {!isLoading && deposits.length === 0 && (
            <div className="flex items-center justify-center h-64">
              <p className="text-gray-500">No manual deposit requests found</p>
            </div>
          )}
