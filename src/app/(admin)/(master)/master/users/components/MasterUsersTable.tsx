...
export default function MasterUsersTable() {
  ...
  const { data, isLoading } = useGetMasterUsersQuery(
    { page: currentPage, perPage: DEFAULT_PER_PAGE, search: debouncedSearchText || undefined },
    { skip: authType !== "master" }
  );

  const [toggleUser] = useToggleMasterUserStatusMutation();
  const [verifyUser, { isLoading: isVerifying }] = useVerifyMasterUserMutation();
  const [resetPassword, { isLoading: isResetting }] = useResetMasterUserPasswordMutation();
  const [addMoneyToUser, { isLoading: isAddingMoney }] = useAddMoneyToMasterUserMutation();
  const [withdrawMoneyFromUser, { isLoading: isWithdrawingMoney }] =
    useWithdrawMoneyFromMasterUserMutation();
  ...
  const handleWithdrawMoney = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedUserId || !withdrawAmount.trim()) return;
    try {
      await withdrawMoneyFromUser({
  ...
  return (
    <div>
      <div className="mb-4 flex items-center justify-between">
        ...
        <Link href="/master/users/create-update" className="flex items-center">
                  ...
                  <TableRow key={user.id}>
                    ...
                    <TableCell className="px-4 py-3 text-theme-sm text-gray-500">
                      <span className="flex items-center gap-2">
                        <Link href={`/master/users/${user.id}/detail`} title="View User Detail">
                          <Eye className="h-5 w-5 text-gray-600 transition-colors hover:text-blue-600" />
                        </Link>
                        <Link
                          href={`/master/users/create-update?id=${user.id}`}
                          title="Edit User"
                        >
                          <svg
        ...
        <div>
          ...
          <form onSubmit={handleVerifyUser} className="space-y-4">
            ...
            <div className="flex items-center justify-end gap-3 border-t border-gray-200 pt-4 dark:border-gray-700">
              <Button type="button" variant="outline" onClick={closeVerifyModal} disabled={isVerifying}>
                Cancel
              </Button>
      ...
      >
        <div>
          ...
          <form onSubmit={handleResetPassword} className="space-y-4">
            ...
            <div className="flex items-center justify-end gap-3 border-t border-gray-200 pt-4 dark:border-gray-700">
              <Button type="button" variant="outline" onClick={closeResetPasswordModal} disabled={isResetting}>
                Cancel
              </Button>
      ...
      >
        <div>
          ...
          <form onSubmit={handleAddMoney} className="space-y-4">
            ...
            <div className="flex items-center justify-end gap-3 border-t border-gray-200 pt-4 dark:border-gray-700">
              <Button type="button" variant="outline" onClick={closeAddMoneyModal} disabled={isAddingMoney}>
                Cancel
              </Button>
      ...
      <Modal
        ...
        onClose={closeWithdrawMoneyModal}
        className="max-w-[500px] p-6 lg:p-8"
      >
        <div>
          <h3 className="mb-2 text-xl font-semibold text-gray-900 dark:text-white">Withdraw Money</h3>
          <p className="mb-6 text-sm text-gray-500 dark:text-gray-400">
          ...
          <form onSubmit={handleWithdrawMoney} className="space-y-4">
            ...
            <div className="flex items-center justify-end gap-3 border-t border-gray-200 pt-4 dark:border-gray-700">
              <Button type="button" variant="outline" onClick={closeWithdrawMoneyModal} disabled={isWithdrawingMoney}>
                Cancel
              </Button>
              <Button type="submit" variant="primary" disabled={isWithdrawingMoney}>
                {isWithdrawingMoney ? "Withdrawing..." : "Withdraw"}
              </Button>
            </div>
          </form>
        </div>
