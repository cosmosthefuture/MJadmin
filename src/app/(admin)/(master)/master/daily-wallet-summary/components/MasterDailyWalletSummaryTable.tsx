...
import { useGetMasterDailyWalletSummaryQuery } from "@/redux/features/masters/MasterWalletSummaryApiSlice";
import { useAppSelector } from "@/redux/hook";
import { useRouter, useSearchParams } from "next/navigation";
import { DEFAULT_PER_PAGE } from "@/lib/constants";

export default function MasterDailyWalletSummaryTable() {
  ...
  const { data, isLoading } = useGetMasterDailyWalletSummaryQuery(
    { page, per_page },
  ...
  const onPageChange = (nextPage: number) => {
    router.push(`/master/daily-wallet-summary?page=${nextPage}&per_page=${per_page}`);
  };
      ...
      <div className="max-w-full overflow-x-auto">
        <div className="min-w-[900px]">
          ...
          {!isLoading && items.length === 0 && (
            <div className="flex items-center justify-center h-64">
              <p className="text-gray-500">No daily wallet summary found</p>
            </div>
          )}
                ...
                <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                  {items.map((item) => (
                    <TableRow key={item.id}>
                      ...
                      <TableCell className="px-4 py-3 text-end tabular-nums text-gray-500 text-theme-sm">
                        {formatAmount(item.total_in)}
                      </TableCell>
                      <TableCell className="px-4 py-3 text-end tabular-nums text-gray-500 text-theme-sm">
                        {formatAmount(item.total_out)}
                      </TableCell>
                      <TableCell className="px-4 py-3 text-end tabular-nums text-gray-500 text-theme-sm">
